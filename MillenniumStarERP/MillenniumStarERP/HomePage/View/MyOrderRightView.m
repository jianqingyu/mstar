...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self = [[NSBundle mainBundle] loadNibNamed:@"MyOrderRightView"
...
#pragma mark -- BaseListBtnView
- (void)setBaseListBtnView{
    CGFloat sWidth = 60;
    ...
    [titleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.topBackView).offset(75);
...
- (void)setBaseViewData:(NSDictionary *)dic{
    NSArray *arr = [ScreenDetailInfo mj_objectArrayWithKeyValuesArray:dic[@"searchKeyword"]];
    NSArray *arrBtn = [ScreenDetailInfo mj_objectArrayWithKeyValuesArray:dic[@"searchScope"]];
    NSArray *arrSea = [SearchDateInfo mj_objectArrayWithKeyValuesArray:dic[@"searchDateScope"]];
...
- (void)pushSearchVC{
    ...
    if (self.back) {
        self.back(@{@"push":cusVc},NO);
    }
//    UIViewController *cur = [ShowLoginViewTool getCurrentVC];
//    [cur.navigationController pushViewController:cusVc animated:YES];
}
