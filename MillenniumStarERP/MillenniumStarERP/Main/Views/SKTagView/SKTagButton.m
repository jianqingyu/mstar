...
+ (instancetype)buttonWithTag:(SKTag *)tag
{
    ...
    if (tag.sTextColor){
        [btn setTitleColor:tag.sTextColor forState:UIControlStateSelected];
    }
    if (tag.Img) {
        [btn setImage:tag.Img forState:UIControlStateNormal];
    }
    if (tag.sImg) {
        [btn setImage:tag.sImg forState:UIControlStateSelected];
    }
    if (tag.bgImg)
    {
    ...
    if (tag.sBgImg)
    {
        [btn setBackgroundImage:tag.sBgImg forState:UIControlStateSelected];
    }
    if (tag.borderColor)
    {
